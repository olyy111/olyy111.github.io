...
function MTween(init){
	...
	var b = {};
	var c = {};
	var d = init.time / 20;//运动时间/20
	for(var s in init.target){ 
		b[s] = css(init.el, s); //获取init.el中的属性放入到b中
		c[s] = init.target[s] - b[s];//算出和目标点的差值放入到c中
	}
	...
	init.el.timer = setInterval(function(){
			...
			if(t>d){
				clearInterval(init.el.timer);
				init.callBack&&init.callBack.call(init.el);//如果回调函数存在执行回调函数
			} else {
				init.callIn&&init.callIn.call(init.el);//如果执行过程中的函数存在执行该函数
				for(var s in b){
					var val = Number((Tween[init.type](t,b[s],c[s],d)).toFixed(2));//获取每一项的值，四舍五入到小数点后两位
					css(init.el, s, val);
				}
			}
		},20
